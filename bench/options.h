...
bool parse_args(options& o, int argc, char** argv) {
	...
	if (cmdl["help"]) {
		std::cerr << "usage [-nx=<image-width 500>] [-ny=<image-height 500>] [-spp=<spp 1>] [-md=<max-depth 50>] [-ns=<num-strips 1>] [-show_image] [-kernel_perf] [-per_iter_perf]" << std::endl;
		return false;
	}
	cmdl({ "nx", "width" }, o.nx) >> o.nx;
	cmdl({ "ny", "height" }, o.ny) >> o.ny;
	cmdl({ "spp", "num_samples" }, o.spp) >> o.spp;
	cmdl({ "ns", "num_strips" }, o.num_strips) >> o.num_strips;
